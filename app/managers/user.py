...
from app.utils.data_conversion import convert_into_dict
...
class UserManager:
    ...
    @staticmethod
    async def get_user_by_id(user_id: str, database):
        """Return a specific user by their email address."""
        user_record = await database.fetch_one(
            User.select().where(User.c.id == user_id)
        )
        return convert_into_dict(data=user_record, exclude_keys=[])
