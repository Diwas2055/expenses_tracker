...
async def get_users(user_id: Optional[UUID] = None, db=Depends(get_database)):
...
async def make_admin(user_id: UUID, db=Depends(get_database)):
...
async def change_password(
    user_id: UUID, user_data: UserChangePasswordRequest, db=Depends(get_database)
):
...
async def ban_user(request: Request, user_id: UUID, db=Depends(get_database)):
...
async def unban_user(request: Request, user_id: UUID, db=Depends(get_database)):
    """Ban the specific user Id.
...
async def edit_user(
    user_id: UUID, user_data: UserEditRequest, db=Depends(get_database)
):
...
async def delete_user(user_id: UUID, db=Depends(get_database)):
